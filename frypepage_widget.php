...
D = {
	w:function(par){
		...
		fileref.setAttribute("href", "<?php echo WP_PLUGIN_URL.'/'.FRYPEFANPAGEF.'/js';?>/widget.css?v=<?php echo VERSION.'x'.date('YmdHis');?>");
		<?php else: ?>
		fileref.setAttribute("href", "<?php echo WP_PLUGIN_URL.'/'.FRYPEFANPAGEF.'/js';?>/widget.css?v=<?php echo VERSION.'x'.date('YmdHis');?>");
		<?php endif; ?>
		...
		html = '<div style="width:'+par['width'] + 'px;"><div class="draugiemwidget_shadbox"><div class="shtext" style="background-image: url('+par['page']['image'] +');">'+par['page']['about']+'<a class="title" href="http://mediabox.lv/a/fryped.php?gotopage='+par['page']['url']+'"><b>'+par['page']['totalfans']+'</b> <?php _e('fans', 'frypepage_widget'); ?></a><div><div class="joinbutton"><a href="http://mediabox.lv/a/fryped.php?gotopage=' + par['page']['url']+ '"></a></div><?php  // @since v0.2 <div style="clear:both;"></div>  ?></div></div>';
		if (par['fans'].length){
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => __('Draugiem.lv/lapas', 'frypepage_widget'), 'name' => __('umbrovskiscom', 'frypepage_widget'), 'wwidth' => '240', 'show_usersfrp' => '0', 'show_cssfrp' =>WP_PLUGIN_URL.'/'.FRYPEFANPAGEF.'/js/widget.css');
