 ...
 * Plugin URI: http://darbi.mediabox.lv/20-draugiem-lvlapas-fanu-wordpress-spraudnis/#utm_source=wordpress&utm_medium=plugin&utm_campaign=meblogfrypepage_load_widgets&utm_content=v016
...
if(!empty($currentLocale)) {
	$moFile = FRYPEFANPAGED. "/lang/frypepage_widget-" . $currentLocale . ".mo";
...
class MeblogFrypePage_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ($title)	echo $before_title . $title . $after_title;
			echo "\n\n<!-- draugiem.lv/lapas fani ".VERSION." via http://umbrovskis.com | MediaBox.lv | SimpleMediaCode.com -->\n".'<div id="pageswidget"></div>'."\n";
		 // * @since 0.2
...
D = {
	w:function(par){
		...
		if (par['fans'].length){
			...
			html += '<div class="dfoot"><a href="#" onClick="dfp_DraugiemSay(); return false"><img src="<?php echo WP_PLUGIN_URL.'/'.FRYPEFANPAGEF.'/img/';?>draugiem-say.png" width="16" height="16" alt="iesaki draugiem" border="0"></a><a href="http://darbi.mediabox.lv/tag/wordpress-spraudnu-izstrade/?utm_campaign=WordPress_Plugins&utm_content=<?php echo FRYPEFANPAGEF.'-v'.VERSION;?>&utm_medium=iconlink" title="WordPress spraudņu un tēmu izstrāde"><img src="<?php echo WP_PLUGIN_URL.'/'.FRYPEFANPAGEF.'/img/';?>mediabox-icon-16sq.png" width="16" height="16" alt="MB" border="0"></a></div></div><?php  //fanlist ?>';
		}
		html += '</div>';
...
<?php echo'<script type="text/javascript" src="http://www.draugiem.lv/'.$name.'/js/fans/?container=pageswidget&count='.$show_usersfrp.'&w='.$wwidth.'"></script>'."\n<!-- draugiem.lv/lapas fani ".VERSION."  beidzas footer -->\n\n";
	...
	function form( $instance ) {
        ...
        <!-- HELP link-->
        <p><a href="http://darbi.mediabox.lv/20-draugiem-lvlapas-fanu-wordpress-spraudnis/?utm_campaign=WordPress_Plugins&utm_content=<?php echo FRYPEFANPAGEF.'-v'.VERSION;?>&utm_medium=textlink" title="draugiem.lv/lapas fanu lapa" target="_blank"><img src="<?php echo WP_PLUGIN_URL.'/'.FRYPEFANPAGEF.'/img/';?>help.png" width="16" height="16" alt="" /><?php _e('Help?', 'frypepage_widget'); ?></a> <?php _e('(in new window/tab)', 'frypepage_widget'); ?></p>
...
function mediabox_feedlink_ffp(){
	...
	$rss = fetch_feed('http://mediabox.lv/rss-tech.php');
...
function fryped_head_ffp(){
	echo "<script type=\"text/javascript\">function dfp_DraugiemSay(titlePrefix ){window.open('http://www.draugiem.lv/say/ext/add.php?title=' + encodeURIComponent(document.title)+'&link=' + encodeURIComponent(window.location) + (titlePrefix ? '&titlePrefix=' + encodeURIComponent( titlePrefix ) : '' ),'','location=1,status=1,scrollbars=0,resizable=0,width=530,height=400'); return false;}</script>";
	}

add_filter('wp_head', 'fryped_head_ffp');
?>