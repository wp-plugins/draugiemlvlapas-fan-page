...
class MeblogFrypePage_Widget extends WP_Widget
{
    ...
    function MeblogFrypePage_Widget()
    {
        ...
        /* Create the widget. */
        $this->WP_Widget(
                'meblogfrypepage-widget', __('Draugiem Lapas Widget', 'frypepage_widget'), $widget_ops, $control_ops
        );
    ...
    function widget($args, $instance)
    {
        ...
        if (!$widgetid || $widgetid == '') {
            $widgetid = md5(mt_rand(100000000, time()));
        }
        if (!$wheight) {
            $wheight = '230';
        }
        // Maybe remove in next release!
        if (!is_numeric($show_pageaboutlenght) || !$show_pageaboutlenght) {
            $show_pageaboutlenght = '200';
        }
        ...
        if ($show_cssfrp):
            ?><link type="text/css" media="all" href="<?php echo $show_cssfrp; ?>?v=<?php echo $widgetid; ?>" /><?php endif; ?>
    ...
    function form($instance)
    {
        ...
        <p><label for="<?php echo $this->get_field_id('show_saytexts'); ?>"><?php _e('Show SAY?', 'frypepage_widget'); ?></label><br />
          <select id="<?php echo $this->get_field_id('show_saytexts'); ?>" name="<?php echo $this->get_field_name('show_saytexts'); ?>" >
            <option value="1"<?php if ($instance['show_saytexts'] == 1) {
            echo ' selected';
        } ?>><?php _e('yes', 'frypepage_widget'); ?></option>
            <option value="0"<?php if ($instance['show_saytexts'] == 0) {
            echo ' selected';
        } ?>><?php _e('no', 'frypepage_widget'); ?></option>
          </select>
        </p>
        ...
        if ($dfp_uid == '' || !$dfp_uid): $dfp_uid = md5(mt_rand(100000000, time()));
        endif;
        ?>
